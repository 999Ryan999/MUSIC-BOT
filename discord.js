...
class Music {
    ...
    async join(msg) {
        ...
        if (msg.member.voice.channel !== null) {
            ...
            msg.channel.send('你沒加入語音頻道我是要怎麼撥音樂啦!!!');
        }
    ...
    async play(msg) {
        ...
        if (!this.connection[guildID]) {
            msg.channel.send('可以用-music join 把我放入頻道喔 :hot_face: ');
            return;
        }

        // 如果 Bot leave 後又未加入語音頻道
        if (this.connection[guildID].status === 4) {
            msg.channel.send('可以用-music join 把我「重新」放入頻道喔 :hot_face: ');
            return;
        }
        ...
        try {
            ...
            if (this.isPlaying[guildID]) {
                msg.channel.send(`色 :underage: 之歌曲加入隊列：${info.title}`);
            } else {
    ...
    playMusic(msg, guildID, musicInfo) {
        ...
        // 提示播放音樂
        msg.channel.send(`播放瑟瑟的音樂 :underage:  ：${musicInfo.name}`);
    ...
    leave(msg) {
        ...
        if (this.connection[msg.guild.id] && this.connection[msg.guild.id].status === 0) {
            ...
            msg.channel.send('我未加入任何頻道(可能是你智商的問題)');
        }
